...
> Immutable reference for Phase 1â€“5 scope. Implementation modules must not silently diverge; changes require audit entry.
...
## Table of Contents
1. Phase 1 â€“ Data Ingestion & Normalization
2. Phase 2 â€“ Baseline Detection & Metrics
3. Phase 3 â€“ Spiking Neural Network (SNN) Prototype
4. Phase 4 â€“ Predictive Fusion & Ensemble Reasoning
5. Phase 5 â€“ Response Orchestration & Integrations
6. Amendments

---

## Phase 1 â€“ Data Ingestion & Normalization
Canonical Goals:
...
- Serialization format evaluation (JSON vs msgpack) â€“ decision: start JSON, allow pluggable codec.

Non-Goals (Phase 1):
- Persistent datastore durability (in-memory acceptable)
- ML inference

## Phase 2 â€“ Baseline Detection & Metrics
Canonical Goals:
...
## Phase 3 â€“ Spiking Neural Network (SNN) Prototype
Canonical Goals:
...
## Phase 4 â€“ Predictive Fusion & Ensemble Reasoning
Canonical Goals:
...
## Phase 5 â€“ Response Orchestration & Integrations
## Amendments

### A14 â€“ Vulnerability Scanning & Exposure Intelligence Pivot (Proposed)
Date: 2025-09-04
...
4. Risk Scoring: Weighted multi-factor (cvss, exploit, exposure, age decay, churn, anomaly correlation, exploit bonus â€“ control reductions) -> normalized risk 0..1 with SLA mapping.
...
Status: Proposed â€“ implementation staged behind feature flag `vuln.scan.enabled`.
Canonical Goals:
...
### A13 (Phase 4 Early Fusion & Temporal Intelligence Expansion) â€“ Pending Hash
Date: 2025-09-03
...
3. Policy Context Provider: Runtime snapshot exporting calibrated quantile deltas (Î”p90, Î”p99) and temporal gating ratio for downstream policy / agent decisions.
4. Insights Engine Scaffold: Heuristic rule set generating structured insight objects (uplift deficit, calibration staleness, suppression elevation, quantile shift). No autonomous actions (advisory only) â€“ prepares ground for future explainability & operator trust.
...
- Insights emitted with deterministic rule evaluations (unit tests confirm scenarios) â€“ no false action triggers.
...
1. Demonstrated uplift stability across â‰¥3 benchmark suites (noise + shift scenarios) with temporal active.
2. Insight accuracy review (manual analyst validation) â€“ <5% misclassification rate in curated scenario set.
...
### A12 (SNN Scaffold Integration & Observability) â€“ Previously Recorded
Date: 2025-09-03
...
Status: Closed â€“ superseded by A13 for fusion evolution context; remains canonical for Phase 3 scope baseline.

### A11 (Phase 2.1 Reliability Scope Introduction) â€“ Pending Implementation
Date: 2025-09-02
...
Scope Additions (Phase 2.1 â€“ Reliability & Evidence):
...
| Tuning | Uplift band (2.0â€“3.5Ã— baseline) achieved with params (`rate_scale=1.03`, `global_shrink=0.4`, `min_floor=0.05`) | Benchmark runs via `scripts/set_params_and_run.py` (audit param log) |
...
| Medium | Auto-Calibration Loop (Threshold) Hardening | Smooth adaptive threshold to maintain target ratio | Converges within N intervals Â± tolerance |
...
1. Implement density cap regression test (now added below in test suite â€” see new test file).
...
|---------------|---------------|-------------------|-----------------|-------|
| Phase 1 â€“ Data Ingestion & Normalization | Neuron-p1-5.txt | TBA | TBA | To be filled after verification |
| Phase 2 â€“ Baseline Detection & Metrics | Neuron-p1-5.txt | TBA | TBA |  |
| Phase 3 â€“ Spiking Neural Network (SNN) Prototype | Neuron-p1-5.txt | TBA | TBA |  |
| Phase 4 â€“ Predictive Fusion & Ensemble Reasoning | Neuron-p1-5.txt | TBA | TBA |  |
| Phase 5 â€“ Response Orchestration & Integrations | Neuron-p1-5.txt | TBA | TBA |  |
